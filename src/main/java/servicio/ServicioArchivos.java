            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (tokens.length >= 6) {
                    try {
                        ...
                        double saldo = Double.parseDouble(tokens[1].trim());
                        long id = Long.parseLong(tokens[2].trim());
                        String nombre = tokens[3].trim();
                        long telefono = Long.parseLong(tokens[4].trim());
                        String zona = tokens[5].trim();
